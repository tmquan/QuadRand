...
class LogVisualizeEpisode (Callback):
    ...
    def _trigger (self):
        ...
        while not done:
            ...
            state_action_values = self.pred (np.expand_dims (obs, 0)) [0]
            state_action_values = np.squeeze (state_action_values)
            print('Step :', step, 'State_action_value:', state_action_values)
            action = np.argmax (state_action_values)
            obs, reward, done, info = player.step (action)
            tot_reward += reward
            print('Choose:', action, 'Current tot reward', tot_reward)
